...
describe('piEnvironment per-session identity', () => {
  ...
  it('keeps collab servers for a session already inside an active Team when the global toggle is off (issue 4734)', async () => {
    ...
    try {
      ...
      const plain = await getPiExtraSpawnConfig([makeProvider()], noopLogger(), {
      ...
      // 已属于 active Team 的 Worker: 下一轮重建描述时仍保留 orca_worker_bridge / cindy_orca
      const worker = await getPiExtraSpawnConfig([makeProvider()], noopLogger(), {
        sessionId: 'pi-4734-worker',
        workingDir: '/repo',
        vendorOptions: { orcaRole: 'worker', orcaWorkflowId: 'team-1', orcaLeadSessionId: 'lead-1' },
      });
      expect(collabNames(worker)).toEqual(['cindy_orca']);
      worker?.disposeSessionCtx?.();

      const lead = await getPiExtraSpawnConfig([makeProvider()], noopLogger(), {
        sessionId: 'pi-4734-lead',
        workingDir: '/repo',
        vendorOptions: { orcaRole: 'lead', orcaWorkflowId: 'team-1', orcaLeadSessionId: 'pi-4734-lead' },
      });
      expect(collabNames(lead)).toEqual(['cindy_orca']);
      lead?.disposeSessionCtx?.();

      // 项目级 / 冻结伙伴的显式停用仍优先于 active Team 豁免
      const frozen = await getPiExtraSpawnConfig([makeProvider()], noopLogger(), {
