...
主动正文的 plain 单段在 MCP 和 Host 两层均限 4096 个 UTF-16 单位，超长在写 claim 和网络调用前拒绝，原幂等键仍可用于修正后的内容。调用者须先分段并为各段保存独立幂等键；HTML 保留 16000 的原始源码预算（标签/实体与解析后长度不同），本次不新增 HTML 解析器或自动分段。
